...
test('production runbook stages WSL repo files for PowerShell scp', () => {
  ...
  assert.ok(runbook.includes('mkdir -p /mnt/c/transfer/inctrak-deploy'));
  assert.ok(runbook.includes('cp docker/inctrak/docker-compose.yml /mnt/c/transfer/inctrak-deploy/docker-compose.yml'));
  assert.ok(runbook.includes('cp inctrak.db/control_plane.sql /mnt/c/transfer/inctrak-deploy/control_plane.sql'));
  assert.ok(runbook.includes('cp inctrak.db/inctrak_feedback.sql /mnt/c/transfer/inctrak-deploy/inctrak_feedback.sql'));
  assert.ok(runbook.includes('cp inctrak.db/inctrak.sql /mnt/c/transfer/inctrak-deploy/inctrak.sql'));
  ...
  assert.ok(runbook.includes('scp "$transfer\\docker-compose.yml" ${server}:/srv/stacks/inctrak/api/docker-compose.yml'));
