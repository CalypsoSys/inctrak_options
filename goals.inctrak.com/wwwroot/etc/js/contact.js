function ContactCtrl($scope, $http, $cookies, $routeParams, $location) {
    ...
    $scope.save_message = "";
    $scope.messageTypes = [];
    $scope.baseGoalGet(" - Loading...", "/api/feedback/message_types/").baseGoalComplete(function (data) {
        $scope.messageTypes = data;
    });

    $scope.baseGoalGet(" - Loading...", "/api/feedback/get_message/").baseGoalComplete(function (data) {
        $scope.formData = data;
    });
