function SchedulesCtrl($scope, $cookies, $http, $routeParams, $location) {
    $scope.$emit('newPageLoaded', {
        ...
        description: 'Create your Schedules.',
    ...
    $scope.baseGoalGet(" - Loading...", "/api/company/schedules/").baseGoalComplete(function (data) {
        $scope.breadCrumbs = [];
    ...
    $scope.loadSchedule = function (key) {
        ...
        if (!uuid || uuid == "not set") {
            ...
            $scope.baseGoalGet(" - Loading...", "/api/company/schedule/" + $scope.scheduleKey + "/" + uuid + "/").
                baseGoalComplete(function (data) {
                    if (data.success) {
                        ...
                        $scope.data = data;
                        if (data && data.Schedule && data.Schedule.START_DATE) {
                            data.Schedule.START_DATE = data.Schedule.START_DATE.split('T')[0];
                        }
                        if (data && data.Schedule && data.Schedule.END_DATE) {
                            data.Schedule.END_DATE = data.Schedule.END_DATE.split('T')[0];
                        }
    ...
    $scope.processForm = function () {
        ...
        if (!uuid || uuid == "not set") {
            ...
            if (!schedule.START_DATE)
                scheduleError += "Start Date ";
            if (!schedule.END_DATE)
                scheduleError += "End Date ";
            ...
            if ( error ) {
                ...
                $scope.preBusy();
                $http.post("/api/company/schedule/", { Key: $scope.scheduleKey, UUID: uuid, Data: schedule }).
    ...
    $scope.removeSchedule = function (key) {
        ...
        if (!uuid || uuid == "not set") {
            ...
            $scope.baseGoalDelete(" - Loading...", "/api/company/schedule/" +$scope.scheduleKey + "/" +uuid + "/").
